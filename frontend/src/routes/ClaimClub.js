	...
	return (
		<>
			{!clubLoading && <div className="bg-neutral-800 mx-auto w-1/2 rounded-lg p-20 my-10 text-neutral-100 text-4xl">
				<div className="text-center mb-10">Registering as Officer for {club.name}</div>
				{error && <div className="text-red-500 my-4 text-base">* {error}</div>}
				<form onSubmit={submitClub} className="flex flex-col gap-4">
					<div className="text-2xl">Name</div>
					<input name='name' value={formState.name} className="bg-neutral-800 border resize-none rounded-md" onChange={handleTextChange}></input>
					...
					<div className="text-2xl">Advisor</div>
...
function DatePicker(props) {
	...
	return (
		<div className="grid grid-cols-7 text-center gap-10">
			<button type="button" onClick={toggleDate(0)} className={`text-lg font-semibold border rounded-md ease-in-out duration-300 ${selectedDates[0] ? 'bg-neutral-100 text-neutral-900 ' : 'bg-neutral-800 text-neutral-100'}`}>S</button>
			<button type="button" onClick={toggleDate(1)} className={`text-lg font-semibold border rounded-md ease-in-out duration-300 ${selectedDates[1] ? 'bg-neutral-100 text-neutral-900 ' : 'bg-neutral-800 text-neutral-100'}`}>M</button>
			<button type="button" onClick={toggleDate(2)} className={`text-lg font-semibold border rounded-md ease-in-out duration-300 ${selectedDates[2] ? 'bg-neutral-100 text-neutral-900 ' : 'bg-neutral-800 text-neutral-100'}`}>T</button>
			<button type="button" onClick={toggleDate(3)} className={`text-lg font-semibold border rounded-md ease-in-out duration-300 ${selectedDates[3] ? 'bg-neutral-100 text-neutral-900 ' : 'bg-neutral-800 text-neutral-100'}`}>W</button>
			<button type="button" onClick={toggleDate(4)} className={`text-lg font-semibold border rounded-md ease-in-out duration-300 ${selectedDates[4] ? 'bg-neutral-100 text-neutral-900 ' : 'bg-neutral-800 text-neutral-100'}`}>T</button>
			<button type="button" onClick={toggleDate(5)} className={`text-lg font-semibold border rounded-md ease-in-out duration-300 ${selectedDates[5] ? 'bg-neutral-100 text-neutral-900 ' : 'bg-neutral-800 text-neutral-100'}`}>F</button>
			<button type="button" onClick={toggleDate(6)} className={`text-lg font-semibold border rounded-md ease-in-out duration-300 ${selectedDates[6] ? 'bg-neutral-100 text-neutral-900 ' : 'bg-neutral-800 text-neutral-100'}`}>S</button>
		</div>
	)
}
	...
	return (
		<div>
			{Object.entries(props.tags).map(([tag, value]) =>
				<button type='button' key={tag} onClick={() => props.handleTagChange(tag)} className={`text-xl block border p-5 rounded-md w-full mb-5 text-left ease-in-out duration-300 ${value ? 'bg-neutral-100 text-neutral-900' : 'bg-neutral-800 text-neutral-100'}`}>
